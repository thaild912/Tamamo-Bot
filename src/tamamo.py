...
<<<<<<< HEAD
    ...
    if debug:
        token = 'NjA1MjE5Mzk5OTgwMjg1OTU1.XT5VGg.PSZLE7dAUrAwUuCUfSZZHeySjn4'
...
<<<<<<< HEAD
    ...
    embed.set_author(name="Tamamo Rabbit")
    embed.set_thumbnail(url=url)
    embed.add_field(name = "<3intro", value = "**A brief introduction about us. Please be serious.**\n120s cooldown.", inline = False)
    embed.add_field(name = "<3welcome", value="**Send a heart warming welcome, not from me.**\n30s cooldown.", inline=False)
    embed.add_field(name = "<3henshin <persona>", value="**Changes persona. Set parameter empty to change to the next available persona. In the case that you need help, which is almost a certainty, then remember that <3henshin help exists.**\n30s cooldown.", inline=False)
    embed.add_field(name = "<3help", value="**You are already here. What are you expecting?\n5s cooldown.", inline=False)
    embed.add_field(name = "<3purge <limit>", value="**Vicchi's cleanup service. Parameter is the number of targets.**\n5s cooldown.", inline=False)
    embed.add_field(name = "<3upcomingbanner <number>", value="**Show <number> of upcoming banner in FGONA.**\n60s cooldown", inline=False)
    embed.add_field(name = "<3selfie", value="**Show a cute picture of the cutest creature in the universe, which happens to share the same appearance as me.**\n10s cooldown", inline=False)
    embed.add_field(name = "<3fetchPDT", value="**Return PDT time for FGONA coordination.**\n5s cooldown", inline=False)
    embed.add_field(name = "<3rateup <servant name>", value="**Show upcomming rateup for a specific servant. Do me a favor and consult <3rateup help before doing anything stupid, will you?**\n30s cooldown.", inline=False)
...
<<<<<<< HEAD
    ...
    if mode == 'OG':
        ...
        if not discovered:
            ...
            await Tamamo.convbox("Or bath first?");    await enhsleep(ctx,1)
            await Tamamo.convbox("Or perhaps...");     await enhsleep(ctx,1)
            await Tamamo.convbox("わ。");               await enhsleep(ctx,1)
            await Tamamo.convbox("た。");               await enhsleep(ctx,1)
            ...
            try:
                ...
                if msg.content.lower().find('dinner') !=-1:
                    ...
                    await enhsleep(ctx,1)
                    await Tamamo.dialogbox("Tamamo-chan~, cooking mode engaged, HENSHIN!")
                    #henshin
                    ...
                    await Tamamo.dialogbox("Or..perhaps...you want us to get in together and clean each other's back, My Husband...?")
                    try:
                        msg = await bot.wait_for('message', check=pred, timeout =30.0)
                    except asyncio.TimeoutError:
                        await Tamamo.dialogbox("Still indecisive?")
                        await enhsleep(ctx, 1)
                        await Tamamo.convobox("Then it's decided. Tamamo will be the one to give My Husband the best back cleaning ever.")
                        await enhsleep(ctx,1)
                        ...
                        if msg.content.lower().find('yes') !=-1:
                            #still blush
                            await Tamamo.dialogbox("Then it's decided. Tamamo will be the one to give My Husband the best back cleaning ever.")
                            await enhsleep(ctx,1)
                            await Tamamo.convbox("And you know it's not limited to back either right?",'*ʷᶦⁿᵏ ʷᶦⁿᵏ')
                            
                        elif msg.content.lower().find('no') !=-1:
                            #sad
                            await Tamamo.convbox("*sob sob. And I was going to give you the best back cleaning ever, My Husband.")
                            #change expression to normal
                            await Tamamo.dialogbox("But well, that alone won't be able to discourage me.")
                            await enhsleep(ctx,1)
                            await Tamamo.convbox('Please get in the bath, My Husband, and in the meanwhile I will prepare you a dinner.')
                            await enhsleep(ctx,1)
                            await Tamamo.convbox("If I can't take down you with my superior housewife back-cleaning skill then I will do it with my cooking skill.")
                            await enhsleep(ctx,1)
                            await Tamamo.convbox("Please look forward to it, My Husband!")
                
                        
                elif msg.content.lower().find('you') !=-1 or msg.content.lower().find('tamamo') !=-1 or msg.content.lower().find('wife') !=-1:
        ...
        await enhsleep(ctx,1)
        await Cat.convbox("You must be hungry. But don't worry because Cat has already cooked you a dinner! Sit down and eat up, Master!", "ʷᵃᵍ ʷᵃᵍ")
        await enhsleep(ctx,1)
        ...
        await enhsleep(ctx,1)
        await Vicchi.convbox("Now if you'll excuse me, I'll help you change your attire, and then we will go out.", "ʰᵃⁿᵈ ʷᵃⁿᵈᵉʳˢ")
            
            
        
    elif mode == 'Rabbit':
        if bond<80:
            await Rabbit.dialogbox("Ara, isn't that my stupid master?")
            await enhsleep(ctx,1)
            await Rabbit.convbox("Welcome home.")
        
        elif bond>=80:                        
            await Rabbit.dialogbox("Ara, looks who have done messing around, %s-kun." %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("Ara, I do not look like I'm was awaiting to your return, you say?")
            await enhsleep(ctx,1)
            await Rabbit.convbox("Why should I, %s-kun?" %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("It's becaise I'm your girlfriend, %s-kun?" %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("Very well. Since you have said such sincere thing, and since I'm such an awesome girlfriend, I will let you go this time.", "ᵈᵒⁿ'ᵗ ᵖᵘˢʰ ʸᵒᵘʳ ˡᵘᶜᵏ ᵃᵍᵃᶦⁿ")
            await enhsleep(ctx,1)
            await Rabbit.convbox("Then, for dinner, your wonderful girlfriend, me, in particular, have prepared the ingredients for your favorite meal. Be grateful, %s-kun." %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("Also, I've gone out of my way prepared the bath for you, so hurry up and get in so I can clean your back, %s-kun." %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("After bath, I shall grant you the privilege to fluff my tail all you want while I cook your dinner, so look forward to it, %s-kun." %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("What will we have for dessert? Ara, aren't you greedy, %s-kun?" %mname)
            await enhsleep(ctx,1)
            await Rabbit.convbox("It will be Me, with chocolate on top, of course.")
            await enhsleep(ctx,1)
        ...
        if bond<80:
            ...
            await enhsleep(ctx,1)
            await Shark.convbox("Looks like you've had a hardworking day. Perhaps you would want a back massage, Master?")
            def pred(m):
                return m.author == ctx.author and m.channel == ctx.channel
            ...
            await enhsleep(ctx,1)
            await Shark.convbox("Now, please don't make a lady wait and get in the bath, Master. Else, you will why people called me 'Beast of Summer ♡'.")
...
<<<<<<< HEAD
    ...
    if message: 
        if not message.lower() in ['og', 'vicchi', 'cat', 'summer', 'rabbit', 'x', 'help']:
        
            await Rabbit.dialogbox("Henshin failed.", "ʳᵉᵃˡˡʸˀ")
            await enhsleep(ctx,1)
            await Rabbit.convbox("There is only a few of us, and most of our names are cutesy, yet you can't remember them.","ᴴᵒʷ ᵘᵗᵗᵉʳˡʸ ᵈᵉˢᵖᶦᶜᵃᵇˡᵉ")
            await enhsleep(ctx,1)
            await Rabbit.convbox("We are dissapointed.","ᵖˡᵉᵃˢᵉ ᵇᵉᵍᵒⁿᵉ")
            await enhsleep(ctx,1)
            await Rabbit.convbox("Go consult !help for more information regarding this command.")
            ...
            embed=discord.Embed(title="Henshin.help", description="Well at least this time that decorative brain of yours know how to ask for help. So I will do you a favor out of kindness and give you the parameters:", color=color)
            embed.set_author(name=name)
            embed.set_thumbnail(url=url)
            embed.add_field(name="Rabbit", value= "**Changes persona to the smart, lovable and superior Tamamo.**", inline=False)
=======
=======
>>>>>>> 028c222f76ca3a16372a5fd81ec2800748d5ebdc
    Knowledge = Servant(ctx, "Tamamo of Knowledge")
...
<<<<<<< HEAD
<<<<<<< HEAD
            embed.add_field(name='Leave the field blank to', value='**Changes persona to the next one available.**', inline=False)
            embed.set_footer(text="And in case that little brain of you forget, all the parameters are not case sensitive.")
            await ctx.send(embed=embed)
        ...
        elif message.lower() == 'rabbit':
            mode = 'Rabbit'
            await henshindiag(Rabbit, ikaros)
...
async def on_command_error(ctx, error):
    ...
    if isinstance(error, commands.CommandOnCooldown):
        await Rabbit.dialogbox("Ara, looks like you can't just stay still for a few second huh?")
        await Rabbit.convbox("Always tampering everywhere..., how childist.",)
        await Rabbit.convbox("Have you ever consider acting like a normal human?")
        print(re.findall(r'\d\d*.\d\d+', str(error))[0])
        
    elif isinstance(error, commands.CommandNotFound):
        await Rabbit.dialogbox("Ara, Congratulations.")
        await Rabbit.convbox("Your stupidity has successfuly exceeded my expectation.")
        await Rabbit.convbox("There are only a few commands, it is that hard to remember?")
        await Rabbit.convbox("...I'm sorry for being so harsh.")
        await Rabbit.convbox("Sorry. I didn't know that the commands's information exceed your brain's capacity.")
        await Rabbit.convbox("To make up for that, I've made a whole new section for the disabled.", "ⁿᵒ ᴵ ᵈᶦᵈⁿ'ᵗ")
        await Rabbit.convbox("Please consult <3help for commands that you can copy and paste,", "ˡᵒʷ⁻ᶜᵃᵖᵃᶜᶦᵗᶦᵉˢ")
        
    elif isinstance(error, commands.MissingRequiredArgument):
        await Rabbit.dialogbox("Hmm? You have finally able to remember the command but not the parameters?")
        await Rabbit.convbox("You know very well that a command is useless without its parameters right?")
        await Rabbit.convbox("Just like how you are in real life.")
...
<<<<<<< HEAD
            servant = Servant(ctx, "Tamamo Rabbit")
            await servant.dialogbox("Has watching porn too much hinders your ability to recognize the right parameter to type?")
            incbond(ctx.author.id, -1)
    if mode == 'Rabbit':
        servant = Servant(ctx, "Tamamo Rabbit")
=======
...
<<<<<<< HEAD
<<<<<<< HEAD
            await servant.dialogbox("If you ask this one more time then the only thing that is nsfw to look at here will be your corpse.")
=======
            await servant.dialogbox(
...
<<<<<<< HEAD
        ...
        await ctx.send(embed = embed)
        for name in cur:
            embed=discord.Embed(title=gacha[str(jpyear)][name]['Name'],description =gacha[str(jpyear)][name]['Date'] ,color=servant.color)
            string =''
            lstring = []
            count = 0
            for item in gacha[str(jpyear)][name]['Servants']:
                if count%10 != 0 or count ==0:
                    string =string + "[%s]" %gacha[str(jpyear)][name]['Servants'][item]['Name'] +'(%s)' %gacha[str(jpyear)][name]['Servants'][item]['URL'] + '\n'
                else:
                    lstring.append(string)
                    string = ''
                    string ="[%s]" %gacha[str(jpyear)][name]['Servants'][item]['Name'] +'(%s)' %gacha[str(jpyear)][name]['Servants'][item]['URL'] + '\n'
                count+=1
            lstring.append(string)
            ...
            if gacha[str(jpyear)][name]['CE']:
                ...
                for item in gacha[str(jpyear)][name]['CE']:
                    if gacha[str(jpyear)][name]['CE'][item]['Name'].find('3') == -1:
                        string2 =string2 + "[%s]" %gacha[str(jpyear)][name]['CE'][item]['Name'] +'(%s)' %gacha[str(jpyear)][name]['CE'][item]['URL']+'\n'
        ...
        if next:
            for name in next:
                embed=discord.Embed(title=gacha[str(jpyear+1)][name]['Name'],description =gacha[str(jpyear+1)][name]['Date'] ,color=servant.color)
                string =''
                for item in gacha[str(jpyear+1)][name]['Servants']:
                    if gacha[str(jpyear+1)][name]['Servants'][item]['Name'].find('3') != -1:
                        continue
                    else:
                        string =string + "[%s]" %gacha[str(jpyear+1)][name]['Servants'][item]['Name'] +'(%s)' %gacha[str(jpyear+1)][name]['Servants'][item]['URL'] + '\n'
                ...
                if gacha[str(jpyear+1)][name]['CE']:
                    ...
                    for item in gacha[str(jpyear+1)][name]['CE']:
                        if gacha[str(jpyear+1)][name]['CE'][item]['Name'].find('3') == -1:
                            string2 =string2 + "[%s]" %gacha[str(jpyear+1)][name]['CE'][item]['Name'] +'(%s)' %gacha[str(jpyear+1)][name]['CE'][item]['URL']+'\n'
...
<<<<<<< HEAD
    servant = Servant(ctx, "Tamamo Rabbit")
    if name.lower() == 'help':
        await servant.dialogbox('This is the command to search for future banner of a certain banner, if you want to go bankrupt.')
        await servant.convbox('The syntax is simple: "<3rateup <servant name>".')
        await servant.convbox("The most preferable format for the servant name is '<number of stars>★ <servant class> <servant full name>'.")
        await servant.convbox("But of course, this is from you so I won't expect too much about you would be able to remember complicated things.")
        await servant.convbox('So I had make another function that return multiple result if your search inquiry is too dumb.')
        ...
        await servant.rabbitbox("As expected from my maker, ", ikaros, '.')
    ...
    if len(candidate) <= 0:
        await servant.dialogbox("No rateup found. Even if there is, you probably won't be able to summon what you want either, so don't waste my time trying again. ")
...
<<<<<<< HEAD
<<<<<<< HEAD
                await servant.dialogbox("The input isn't an integer number in integer number format. Stop wasting my time with your stupidity.")
                await servant.convbox('Bye.')
            else:
                if x>len(candidate) or x<=-1:
                    await servant.dialogbox("Even a 4 years old kid can count better than you. Go back to kindergarten and ask them to teach you.")
=======
=======
>>>>>>> 028c222f76ca3a16372a5fd81ec2800748d5ebdc
                await servant.dialogbox(
...
<<<<<<< HEAD
<<<<<<< HEAD
        await Tamamo.convbox("I am sure that you will be able to find someone who will be a better wife for you more than me!")
=======
        await Tamamo.convbox(
...
<<<<<<< HEAD
        ...
        embed.add_field(name="Motto:", value = "Your reliable Caster, specified in martial arts, magic and curses.", inline=False)
...
<<<<<<< HEAD
    ...
    if page == 2:
        ...
        await enhsleep(ctx, 1)
        await Cat.convbox("Since nine-tailed Tamamo splited into nine of us, each of us Alter Ego received aspects of the original.",'ʷᵃᵍ ʷᵃᵍ')
        await enhsleep(ctx, 1)
...
<<<<<<< HEAD
    ...
    if status:
        await enhsleep(ctx, 1)
        await Rabbit.dialogbox("You have already adopted us.")
        return
    
    if 10>=sanction>=8:
        await Rabbit.dialogbox("Get lost.")
        decsanction(ctx.author.id)
        
    elif 7>=sanction>=5:
        await Rabbit.dialogbox("You still have the nerve to hang around us? After all of that?")
        decsanction(ctx.author.id)
        
    elif 4>=sanction>=2:
        await Rabbit.dialogbox("We are dissapointed in you. We've tried to be serious, but you didn't take us seriously.")
        decsanction(ctx.author.id)
        
    elif sanction == 1:
        decsanction(ctx.author.id)
        await Rabbit.dialogbox("Very well. The Tamamo Council have passed an agreement to give you permission to adopt us again.")
        await enhsleep(ctx, 1)
        await Rabbit.convbox("Please be sincere this time, and don't make us revoke our change.")
        await enhsleep(ctx, 1)
        await Rabbit.rabbitbox("Welcome back, Master ", '<@%s>' %str(ctx.author.id), '.')
...
<<<<<<< HEAD
        ...
        await enhsleep(ctx, 1)
        await Shark.convbox("The only different this time is instead of beach sandals, it will be APHE titanium high heels, Mikon!")
...
async def addemoji(ctx, emoji = '', url = ''):
    ...
    if not checkemoji(emoji) and not checkurl(url):
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("Congratulation %s, you have reached a new level of stupidity. Hooray." %username)
        await enhsleep(ctx,1)
        await Rabbit.convbox("Not only did you get the emoji syntax wrong, the 'thing' you inputted in url is not even a gif url.")
        return 0;
    
    elif not checkemoji(emoji):
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("...This is not even a emoji syntax.")
        return 0
        
    elif not checkurl(url):
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("I'm not going to waste my memory remembering your random shits.")
        await enhsleep(ctx,1)
        await Rabbit.convbox("Gif URLs only, is it that hard to understand?")
        return 0
    
    
    if emoji.lower().find("padoru") != -1 or emoji.lower().find("pad0ru") != -1:
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("No padoru.")
        return 0
...
<<<<<<< HEAD
    ...
    if not valid:
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("Invalid URL.")
        await enhsleep(ctx, 0.5)
        await Rabbit.convbox("Incompetent.")
        return 0
...
<<<<<<< HEAD
        ...
        await enhsleep(ctx,1)
        await Rabbit.dialogbox("There is no emoji under %s to be deleted." %emoji)
        await enhsleep(ctx,1)
        await Rabbit.convbox("Perhaps that it's you that should be deleted?")
...
<<<<<<< HEAD
    ...
    embed = discord.Embed(title="Stats for %s" %username, color= Rabbit.color)
    embed.set_author(name=Rabbit.name, icon_url = Rabbit.url)
