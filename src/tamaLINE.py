...
class Servant:
    ...
    async def boorubox(self, source, file_url, message=None):
        ...
        if source.find("e-hentai") != -1:
            embed.add_field(name="\u200b", value="Please use _[SauceNao](https://saucenao.com/index.php)_ for source.",
                            inline=False)
        else:
    ...
    if servant.name == 'Tamamo no Mae':
        ...
        if bond < 80:
            ...
            await servant.convbox(
                "The weather is beautifu today, so why not change into swimsuit and enjoy ourselves on the beach, Master!")
            await enhsleep(ctx, 1)
        elif bond >= 80:
            await servant.dialogbox("Hello Master %s! Tamamo Shark is here~!" % mname)
            await enhsleep(ctx, 1)
            await servant.convbox(
                "What? You are asking why I used the nickname 'Tamamo Shark' instead of 'Tamamo Summer'?")
            await enhsleep(ctx, 1)
            ...
            await enhsleep(ctx, 1)
            await servant.imagebox("It would be a waste...to not devour you all...m.a.s.t.e.r...",
                                   'https://i.imgur.com/3pXbUlU.png', 'ᵘᶠᵘᶠᵘ♡')
        ...
        await enhsleep(ctx, 1)
        await servant.convbox(
            "Whether it's office works, infiltration, espionage or assassinations, I will get it done.")
        ...
        if bond < 80:
            ...
            await enhsleep(ctx, 1)
            await servant.knowledgebox(
                "Ara, I see that you have been acting unbelievably stupid and reckless since I were gone, right ", tag,
                "?")
            await enhsleep(ctx, 1)
            await servant.knowledgebox(
            ...
            await servant.knowledgebox(
                ...
                tag, ".")
            await enhsleep(ctx, 1)
            await servant.knowledgebox("What, I'm being to harsh on you, ", tag, "?")
            await enhsleep(ctx, 1)
            await servant.knowledgebox(
                "Are you perhaps expecting me to be sweeter toward you because I'm your girlfriend, ", tag, "?")
            await enhsleep(ctx, 1)
            ...
            await servant.knowledgebox(
                ...
                tag, "?")
            await enhsleep(ctx, 1)
            await servant.knowledgebox("So stop complaining, be grateful and do as I said, ", tag, ".")
            await enhsleep(ctx, 1)
            ...
            await enhsleep(ctx, 1)
            await servant.knowledgebox(
                "On the second thought, I might actually say that. Since your IQ is indeed that low after all, ", tag,
                ".")
...
async def pagesethelper(bot, ctx, maxpage):
    ...
    await enhsleep(ctx, 1)
    await Knowledge.convbox("Please enter a number after this line (max page number is %d):" % maxpage,
                            '²⁰ˢ ᶜᵒᵒˡᵈᵒʷⁿ ⁻ ᶠᵃᶦˡ ᵗᵒ ᵉⁿᵗᵉʳ ᵗʰᵉ ʳᶦᵍʰᵗ ᶠᵒʳᵐᵃᵗ ʷᶦˡˡ ᵇᵉ ʰᵉᵃᵛʸ ˢᵃⁿᶜᵗᶦᵒⁿᵉᵈ')
    ...
    try:
        ...
        try:
            ...
            if pnum < 0:
                ...
                await Knowledge.dialogbox("Page number should be between 1 and %d." % maxpage)
                await enhsleep(ctx, 1)
                await Knowledge.convbox("Please try again later.")
                return False
            else:
...
async def checkpoint(ctx, bondreq):
    ...
    if not adopted:
        await enhsleep(ctx, 1)
        await servant.dialogbox('Sorry, but you will need to adopt us first for us to use this function.',
                                'Consult <3adopt for more information.')
        return False
    if bond < bondreq:
        if bond == 0:
            pass
        await enhsleep(ctx, 1)
        await servant.dialogbox("Sorry, you need to reach bond level %d to use this function." % (bondreq // 10),
                                'Consult <3bond for more information.')
        return False
    return True
